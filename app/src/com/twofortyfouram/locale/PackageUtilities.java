...
/**
 * A simple utility class to find a package that is compatible with hosting the
 * Locale Developer Platform.
 */
/*
...
public final class PackageUtilities {
	...
	/**
	 * @return a list wrapped in {@link Collections#unmodifiableList(List)} that
	 *         represents the set of Locale-compatible packages.
	 */
	private static Set<String> constructPackageSet() {
		...
		/*
		 * Note: Tasker is not 100% compatible with Locale's plug-in API, but it
		 * is close enough that these packages are enabled. Tasker's known
		 * incompatibilities are documented on the Tasker website.
		 */
	 ...
	 * 
	 * @param manager
	 *            an instance of {@code PackageManager}. Cannot be null.
	 * @param packageHint
	 *            hint as to which package should take precedence. This
	...
	public static String getCompatiblePackage(final PackageManager manager,
		...
		/*
		 * The interface for this method makes no guarantees as to which host
		 * will be returned. However the implementation is more predictable.
		 */

		final List<PackageInfo> installedPackages = manager
