...
public final class PackageUtilities {
   ...
   */
  /*
   * This is NOT a public field and is subject to change in future releases of the Developer
   * Platform. A conscious design decision was made to use hard-coded package names, rather than
   * dynamic discovery of packages that might be compatible with hosting the Locale Developer
   * Platform API. This is for two reasons: to ensure the host is implemented correctly (hosts must
   * pass the extensive Locale Platform Host compatibility test suite) and to prevent malicious
   * applications from crashing plug-ins by providing bad values. As additional apps implement the
   * Locale Developer Platform, their package names will also be added to this list.
   */
  /*
   * Note: this is implemented as a Set<String> rather than a String[], in order to enforce
   * immutability.
   */
  ...
  private static Set<String> constructPackageSet() {
    ...
    /*
     * Note: Tasker is not 100% compatible with Locale's plug-in API, but it is close enough that
     * these packages are enabled. Tasker's known incompatibilities are documented on the Tasker
     * website.
     */
