...
public final class Intent {
   ...
   * process a requery. A {@code BroadcastReceiver} must return within 10 seconds, otherwise it will
   ...
   * was previously set by the {@code Activity} result of {@link #ACTION_EDIT_SETTING}.
   * <p>
   * There SHOULD be only one {@code Activity} per APK that implements this {@code Intent}. If a
   * single APK wishes to export multiple plug-ins, it MAY implement multiple Activity instances
   * that implement this {@code Intent}, however there must only be a single
   * {@link #ACTION_FIRE_SETTING} receiver. In this scenario, it is the responsibility of the
   * Activities to store enough data in {@link #EXTRA_BUNDLE} to allow this receiver to disambiguate
   * which "plug-in" is being fired. To avoid user confusion, it is recommended that only a single
   * plug-in be implemented per APK.
   * 
   ...
   * that was previously set by the {@code Activity} result of {@link #ACTION_EDIT_SETTING}.
   * <p>
   ...
   * When Locale broadcasts this {@code Intent}, it will be sent directly to the package and class
   * of the plug-in's {@code Activity}. The {@code Intent} may contain a store-and-forward
   * {@link #EXTRA_BUNDLE} that was previously set by the {@code Activity} result of
   * {@link #ACTION_EDIT_CONDITION}.
   * <p>
   * There SHOULD be only one {@code Activity} per APK that implements this {@code Intent}. If a
   * single APK wishes to export multiple plug-ins, it MAY implement multiple Activity instances
   * that implement this {@code Intent}, however there must only be a single
   * {@link #ACTION_QUERY_CONDITION} receiver. In this scenario, it is the responsibility of the
   * Activities to store enough data in {@link #EXTRA_BUNDLE} to allow this receiver to disambiguate
   * which "plug-in" is being queried. To avoid user confusion, it is recommended that only a single
   * plug-in be implemented per APK.
   * 
   * @see Intent#EXTRA_BUNDLE
   * @see Intent#EXTRA_STRING_BREADCRUMB
   ...
   * When Locale broadcasts this {@code Intent}, it will be sent directly to the package and class
   * of the plug-in's {@code BroadcastReceiver}. The {@code Intent} will contain a
   * {@link #EXTRA_BUNDLE} that was previously set by the {@code Activity} result of
   * {@link #ACTION_EDIT_CONDITION}.
   * <p>
   * Since this is an ordered broadcast, the receiver is expected to set an appropriate result code
   * from {@link #RESULT_CONDITION_SATISFIED}, {@link #RESULT_CONDITION_UNSATISFIED}, and
   ...
   * again, Locale will send {@link #ACTION_QUERY_CONDITION}.
   * <p>
   * The extra {@link #EXTRA_ACTIVITY} MUST be included, otherwise Locale will ignore this
   * {@code Intent}.
   * <p>
   ...
   * <p>
   * Plug-ins MUST NOT store {@link Parcelable} objects in this {@code Bundle} , because
   * {@code Parcelable} is not a long-term storage format. Also, plug-ins MUST NOT store any
   * serializable object that is not exposed by the Android SDK.
   * <p>
   ...
   * (base-10). Although the maximum size is about 100 kilobytes, plug-ins SHOULD keep Bundles much
   * smaller for performance and memory usage reasons.
   */
