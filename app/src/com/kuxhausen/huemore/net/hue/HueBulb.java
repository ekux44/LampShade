...
public class HueBulb implements NetworkBulb {
	...
	public HueBulb(Context c, Long bulbBaseId, String bulbName, String bulbDeviceId, HueBulbData bulbData, HubConnection hubConnection, int currentMaxBri) {
		mContext = c;
		mBaseId = bulbBaseId;
		mName = bulbName;
	...
	public void setCurrentMaxBrightness(int maxBri) {
		...
		if(mCurrentMaxBri != maxBri){
			...
			mContext.getContentResolver().update(DatabaseDefinitions.NetBulbColumns.URI, cv, DatabaseDefinitions.NetBulbColumns._ID + " =?", selectionArgs);
