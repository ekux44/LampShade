...
public class UnlocksDialogFragment extends DialogFragment implements OnClickListener {
    ...
    switch (v.getId()) {
      case R.id.fiveMoreButton:
        if (bm.lastQuerriedInventory == null)
          ...
          if (!bm.lastQuerriedInventory.hasPurchase(PlayItems.FIVE_BULB_UNLOCK_1))
            bm.mPlayHelper.launchPurchaseFlow(this.ma, PlayItems.FIVE_BULB_UNLOCK_1, 10009,
                mPurchaseFinishedListener, "");
          else
            Toast.makeText(ma, ma.getString(R.string.unlock_pro_already), Toast.LENGTH_SHORT)
                .show();
        }
        break;
    }
  ...
  IabHelper.OnIabPurchaseFinishedListener mDonatePurchaseFinishedListener =
      new IabHelper.OnIabPurchaseFinishedListener() {
        @Override
        public void onIabPurchaseFinished(IabResult result, Purchase purchase) {
