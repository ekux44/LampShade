...
public class RequestQueue {
   ...
   *
   * <ul> <li>containsKey(cacheKey) indicates that there is a request in flight for the given cache
   * key.</li> <li>get(cacheKey) returns waiting requests for the given cache key. The in flight
   * request is <em>not</em> contained in that list. Is null if no requests are staged.</li> </ul>
   */
    ...
    if (request.shouldCache()) {
      synchronized (mWaitingRequests) {
        ...
        if (waitingRequests != null) {
          if (VolleyLog.DEBUG) {
            VolleyLog.v("Releasing %d waiting requests for cacheKey=%s.", waitingRequests.size(),
                        cacheKey);
          }
