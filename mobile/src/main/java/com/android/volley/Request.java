...
public abstract class Request<T> implements Comparable<Request<T>> {
   ...
   * entry will be stored here so that in the event of a "Not Modified" response, we can be sure it
   ...
   * responses is provided by subclasses, who have a better idea of how to deliver an already-parsed
   * response.
   */
  ...
  /**
   * Returns a Map of POST parameters to be used for this request, or null if a simple GET should be
   * used. Can throw {@link AuthFailureError} as authentication may be required to provide these
   * values.
   *
   * <p> Note that only one of getPostParams() and getPostBody() can return a non-null value. </p>
   *
   * @throws AuthFailureError In the event of auth failure
   ...
   *
   * <p> This controls both encodings: <ol> <li>The string encoding used when converting parameter
   * names and values into bytes prior to URL encoding them.</li> <li>The string encoding used when
   * converting the URL encoded parameters into a raw byte array.</li> </ol>
   *
  ...
  /**
   * Returns which encoding should be used when converting POST or PUT parameters returned by {@link
  ...
  /**
   * Priority values. Requests will be processed from higher priorities to lower priorities, in FIFO
   * order.
   */
  ...
  /**
   * Mark this request as having a response delivered on it. This can be used later in the request's
   * lifetime for suppressing identical responses.
   */
