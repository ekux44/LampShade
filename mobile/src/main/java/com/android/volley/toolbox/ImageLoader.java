 ...
 * pass in the default image listener provided by {@link ImageLoader#getImageListener(ImageView,
...
public class ImageLoader {
   ...
   *
   * The call flow is this: 1. Upon being attached to a request, onResponse(response, true) will be
   * invoked to reflect any cached data that was already available. If the data was available,
  ...
  public interface ImageListener extends ErrorListener {
     ...
     *
     * @param response    Holds all information pertaining to the request, as well as the bitmap (if
     *                    it is loaded).
     * @param isImmediate True if this was called during ImageLoader.get() variants. This can be
     *                    used to differentiate between a cached image loading and a network image
     *                    loading in order to, for example, run an animation to fade in network
     *                    loaded images.
     */
    public void onResponse(ImageContainer response, boolean isImmediate);
  }
  ...
  public class ImageContainer {
    ...
    /**
     * The most relevant bitmap for the container. If the image was in cache, the Holder to use for
     * the final bitmap (the one that pairs to the requested URL).
     */
