...
public class Utils {
  ...
  /**
   * Inspired by
   * https://github.com/PhilipsHue/PhilipsHueSDK-iOS-OSX/blob/master/ApplicationDesignNotes
    ...
    if (1667 <= ct && ct <= 4000) {
      xc =
          -0.2661239 * (Math.pow(10, 9) / Math.pow(ct, 3)) - 0.2343580
              * (Math.pow(10, 6) / Math.pow(ct, 2)) + 0.8776956 * (Math.pow(10, 3) / ct)
              + 0.179910d;
    } else if (4000 < ct && ct < 25000) {
      xc =
          -3.0258469 * (Math.pow(10, 9) / Math.pow(ct, 3)) + 2.1070379
              * (Math.pow(10, 6) / Math.pow(ct, 2)) + 0.2226347 * (Math.pow(10, 3) / ct)
              + 0.240390d;
    }
    if (1667 <= ct && ct <= 2222) {
      yc =
