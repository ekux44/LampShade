...
public class NetworkMethods {
  ...
  public static void preformTransmitPendingState(Route route, String hash, Context context,
      RequestQueue queue, HubConnection connection, PendingStateChange pState) {
    if (queue == null)
      return;
    ...
    GsonRequest<LightsPutResponse[]> req =
        new GsonRequest<LightsPutResponse[]>(Method.PUT, url, gson.toJson(pState.sentState),
            LightsPutResponse[].class, null, new StateSuccessListener(connection, pState, route),
  ...
  public static void PreformGetBulbAttributes(Route route, String hash, Context context,
    ...
    GsonRequest<BulbAttributes> req =
        new GsonRequest<BulbAttributes>(Method.GET, url, null, BulbAttributes.class, null,
            new BulbAttributesSuccessListener(monitor, listener, bulb, route),
  ...
  public static void PreformSetBulbAttributes(Route route, String hash, Context context,
      RequestQueue queue, ConnectionMonitor monitor, int bulbNum, BulbAttributes bulbAtt) {
    ...
    GsonRequest<LightsPutResponse[]> req =
        new GsonRequest<LightsPutResponse[]>(Method.PUT, url, gson.toJson(bulbAtt),
            LightsPutResponse[].class, null, new BasicSuccessListener<LightsPutResponse[]>(monitor,
                route), new BasicErrorListener(monitor, route));
    req.setTag("");
    queue.add(req);
  }

  public static void PreformGetBulbList(Route route, String hash, Context context,
      RequestQueue queue, ConnectionMonitor monitor, OnBulbListReturnedListener listener) {
    if (queue == null)
      return;
    ...
    GsonRequest<BulbList> req =
        new GsonRequest<BulbList>(Method.GET, url, null, BulbList.class, null,
            new BulbListSuccessListener(monitor, listener, context, route), new BasicErrorListener(
                monitor, route));
    req.setTag("");
    queue.add(req);
  }
